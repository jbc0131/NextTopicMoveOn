...
The application should treat each imported raid as two layers:

- raw import datasets from Warcraft Logs
- derived analytics computed from those datasets

This keeps the architecture stable as more legacy categories are ported.

Current implemented derived categories:

- engineering damage taken
- oil of immolation damage taken
- missing permanent enchants
- missing temporary weapon enchants
- class-aware suboptimal temporary weapon enchant detection
- gem quality checks using the legacy common/uncommon/rare thresholds
- player buff aura inventory from WCL buff tables

Legacy categories still to port:

- broader damage taken breakdowns
- config-driven buffs and consumables coverage
- drums effectiveness from event-level buff windows
- debuff uptime and application checks
- interrupts
- healing breakdowns
- gear issues beyond enchant/gem and temporary weapon checks
- shadow resistance
