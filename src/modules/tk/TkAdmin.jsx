...
// ── Roster sidebar ────────────────────────────────────────────────────────────
function TkRosterPanel({ roster, roleFilter, setRoleFilter, onDragStart, onAddManual, onRemove }) {
...
export default function TkAdmin({ teamId }) {
  ...
  const load = useCallback(async () => {
    let s = null;
    if (FIREBASE_OK) {
      try { s = await fetchTkState(teamId); } catch (e) { console.warn(e); }
    }
  ...
  useEffect(() => {
    load();
    document.title = `NTMO TK Admin – ${teamId === "team-dick" ? "Team Dick" : "Team Balls"}`;
  }, [teamId, load]);
  ...
  useEffect(() => {
    ...
    autoSaveTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        await saveTkState(state, teamId);
  ...
  }, [roster, assignments, textInputs, dividers, teamId]);
  ...
  return (
    <AppShell teamId={teamId} adminMode>
      <ConfirmDialog
        ...
        message="This will remove all TK assignments. Cannot be undone."
      ...
      {showImport && (
        <div style={{ padding: space[3], background: surface.panel, borderBottom: `1px solid ${border.subtle}` }}>
          ...
          <textarea
            placeholder="Paste TK roster JSON…"
      ...
      {roster.length === 0 ? (
        <EmptyState icon="⚡" title="No TK roster" message="Import a 25-man JSON roster to get started" action="Import JSON" onAction={() => setShowImport(true)} />
      ) : (
