...
import {
  getColor, getSpecDisplay, SSC_BOSSES,
...
import {
  fetchSscState, subscribeToSscState, isFirebaseConfigured,
...
export default function SscPublic({ teamId }) {
  ...
  useEffect(() => {
    document.title = `NTMO SSC – ${teamId === "team-dick" ? "Team Dick" : "Team Balls"}`;
    if (!FIREBASE_OK) { setLoading(false); return; }

    setLoading(true); setLiveSync(false); setData(null);
    const unsub = subscribeToSscState(teamId, snap => {
      setData(snap); setLoading(false); setLiveSync(true); setLastUpdate(new Date());
    });
    fetchSscState(teamId).then(d => { if (d) { setData(d); setLoading(false); } }).catch(() => {});
    return () => unsub();
  }, [teamId]);
  ...
  return (
    <AppShell teamId={teamId}>
      <ModuleHeader
        ...
        breadcrumb={`${teamId === "team-dick" ? "Team Dick" : "Team Balls"} / SSC`}
        mobileActions={<>
