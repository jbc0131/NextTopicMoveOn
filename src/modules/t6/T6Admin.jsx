...
import { T6_RAIDS, rowsByKey } from "./t6Raids";

const FIREBASE_OK = isFirebaseConfigured();

// ── Assignment row ────────────────────────────────────────────────────────────
// Same row as SSC, plus the T6 config fields: `max` (slot capacity), `ordered`
// (chips read as "A › B › C"), `note` (helper line), `textLong` (full-width text).
function AssignmentRow({ rowCfg, assignedIds, textValues, roster, onDrop, onClear, onTextChange, onDragStart }) {
  ...
  const isTextOnly = rowCfg.textInput && rowCfg.textLong;
  const isFull     = rowCfg.max > 1 && slots.length >= rowCfg.max;
  ...
  return (
    <div
      ...
      style={{
        ...
        background: over && !isTextOnly ? `${accent.blue}12` : "transparent",
        borderLeft: `2px solid ${over && !isTextOnly ? accent.blue : border.subtle}`,
        borderBottom: `1px solid ${border.subtle}`,
        transition: "all 0.1s",
      }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: space[3], minHeight: layout.rowHeight - 2 }}>
        ...
        {isTextOnly ? (
          <textarea
            ...
            rows={2}
            style={{ ...inputStyle, flex: 1, height: "auto", minHeight: 44, padding: space[2], resize: "vertical", fontSize: fontSize.xs, lineHeight: 1.45 }}
          />
        ) : (
          <div style={{ flex: 1, display: "flex", flexWrap: "wrap", gap: space[1], alignItems: "center" }}>
            {slots.map((slot, i) => {
              ...
              return (
                <div key={slot.id} style={{ display: "inline-flex", alignItems: "center", gap: space[1] }}>
                  {rowCfg.ordered && i > 0 && (
                    <span style={{ color: text.disabled, fontSize: fontSize.sm, fontFamily: font.sans }}>›</span>
                  )}
                  <span
            ...
            {over && (
              <span style={{ fontSize: fontSize.xs, color: isFull ? intent.warning : accent.blue, fontStyle: "italic" }}>
                {isFull ? "slot full" : "drop here"}
              </span>
            )}
          </div>
        )}
        ...
        {rowCfg.max > 1 && (
          <span style={{ fontSize: 9, fontFamily: font.mono, color: isFull ? text.muted : text.disabled, flexShrink: 0 }}>
            {slots.length}/{rowCfg.max}
          </span>
        )}

        {rowCfg.textInput && !rowCfg.textLong && (
          <input
            value={textValues?.[rowCfg.key] ?? ""}
            onChange={e => onTextChange && onTextChange(rowCfg.key, e.target.value)}
            placeholder={rowCfg.hint || "Notes…"}
      ...
      {rowCfg.note && (
        <div style={{ fontSize: 10, color: text.muted, fontFamily: font.sans, fontStyle: "italic", paddingLeft: 2, paddingBottom: 2 }}>
          {rowCfg.note}
        </div>
      )}
    </div>
...
// ── Roster sidebar ────────────────────────────────────────────────────────────
function T6RosterPanel({ roster, roleFilter, setRoleFilter, onDragStart, onAddManual, onRemove }) {
...
// ── Main T6Admin ──────────────────────────────────────────────────────────────
// `raid` selects the T6 module: "hyjal" (Mount Hyjal) or "bt" (Black Temple).
export default function T6Admin({ teamId, raid }) {
  const cfg       = T6_RAIDS[raid];
  const BOSSES    = cfg.bosses;
  const MODULE_KEY = cfg.key;
  const ROW_CFG   = useMemo(() => rowsByKey(BOSSES), [BOSSES]);
  const DEFAULTS  = useMemo(() => defaultTextInputs(BOSSES), [BOSSES]);

  const [roster,        setRoster]        = useState([]);
  const [assignments,   setAssignments]   = useState({});
  const [textInputs,    setTextInputs]    = useState({});
  const [dividers,      setDividers]      = useState([]);
  const [activeBoss,    setActiveBoss]    = useState(BOSSES[0].id);
  ...
  const load = useCallback(async () => {
    let s = null;
    if (FIREBASE_OK) {
      try { s = await fetchRaidModuleState(teamId, MODULE_KEY); } catch (e) { console.warn(e); }
    }
    if (!s) s = loadState(teamId, MODULE_KEY);
    if (s) {
      setRoster(s.roster || []);
      setAssignments(s.assignments || {});
      // Seed the spec's default note text for any row never edited yet.
      setTextInputs({ ...DEFAULTS, ...(s.textInputs || {}) });
      setDividers(s.dividers || []);
    } else {
      setRoster([]); setAssignments({}); setTextInputs({ ...DEFAULTS }); setDividers([]);
    }
  }, [teamId, MODULE_KEY, DEFAULTS]);

  useEffect(() => {
    setActiveBoss(BOSSES[0].id);
    load();
    document.title = `NTMO ${cfg.title} Admin – ${teamId === "team-dick" ? "Team Dick" : "Team Balls"}`;
  }, [teamId, load, BOSSES, cfg.title]);
  ...
  useEffect(() => {
    ...
    autoSaveTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        await saveRaidModuleState(state, teamId, MODULE_KEY);
  ...
  }, [roster, assignments, textInputs, dividers, teamId, MODULE_KEY]);
  ...
  const handleImportJSON = useCallback((text) => {
    try {
      ...
      // Guarantee unique roster ids — imported JSON sometimes reuses an id across
      // two players, which would make every assignment for it resolve to whoever
  ...
  const handleDrop = useCallback((key) => {
    ...
    const max = ROW_CFG[key]?.max;
    setAssignments(prev => {
      const asArray = v => (v ? (Array.isArray(v) ? v : [v]) : []);
      const existing = asArray(prev[key]);
      if (existing.includes(playerId) && dragSourceKey === key) return prev;

      // Capacity: a single slot is replaced by the incoming player; a wider slot
      // that is already full rejects the drop (double-click a chip to free one).
      const alreadyIn = existing.includes(playerId);
      if (!alreadyIn && max && existing.length >= max && max > 1) return prev;

      let next = { ...prev };
      if (dragSourceKey && dragSourceKey !== key) {
        const srcUpdated = asArray(next[dragSourceKey]).filter(id => id !== playerId);
        if (srcUpdated.length === 0) delete next[dragSourceKey]; else next[dragSourceKey] = srcUpdated;
      }
      if (!alreadyIn) {
        next[key] = max === 1 ? [playerId] : [...existing, playerId];
      }
      return next;
    });
    setDragSlot(null); setDragSourceKey(null);
  }, [dragSlot, dragSourceKey, ROW_CFG]);
  ...
  const currentBoss = BOSSES.find(b => b.id === activeBoss) || BOSSES[0];
  ...
  return (
    <AppShell teamId={teamId} adminMode>
      <ConfirmDialog
        ...
        message={`This will remove all ${cfg.title} assignments. Cannot be undone.`}
        confirmLabel="Clear All"
        dangerous
        onConfirm={() => { setAssignments({}); setTextInputs({ ...DEFAULTS }); setConfirmClearOpen(false); }}
      ...
      {showImport && (
        <div style={{ padding: space[3], background: surface.panel, borderBottom: `1px solid ${border.subtle}` }}>
          ...
          <textarea
            placeholder={`Paste ${cfg.title} roster JSON…`}
      ...
      {roster.length === 0 ? (
        <EmptyState icon={cfg.empty} title={`No ${cfg.title} roster`} message="Import a 25-man JSON roster to get started" action="Import JSON" onAction={() => setShowImport(true)} />
      ) : (
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: space[4] }}>
            {/* Boss tab bar — trash first, then bosses in kill order */}
            <div style={{
