...
function PublicRow({ rowCfg, ids, textValues, roster, searchName }) {
  ...
  // Fall back to the config default when the raid leader hasn't typed anything.
  const noteText = rowCfg.textInput ? (textValues?.[rowCfg.key] ?? rowCfg.default ?? "") : null;
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: space[2], minHeight: layout.rowHeight - 2 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
          {slots.map((slot, i) => (
            <span key={slot.id} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
              {rowCfg.ordered && i > 0 && (
                <span style={{ color: text.disabled, fontSize: fontSize.sm, fontFamily: font.sans }}>›</span>
              )}
              <PlayerChip slot={slot} searchName={searchName} />
            </span>
          ))}
      ...
      {rowCfg.note && (
        <div style={{ fontSize: 10, color: text.muted, fontFamily: font.sans, fontStyle: "italic", paddingBottom: 2 }}>
          {rowCfg.note}
        </div>
      )}
    </div>
...
export default function T6Public({ teamId, raid }) {
  ...
  useEffect(() => {
    document.title = `NTMO ${cfg.title} – ${teamId === "team-dick" ? "Team Dick" : "Team Balls"}`;
    setActiveBoss(BOSSES[0].id);
    if (!FIREBASE_OK) { setLoading(false); return; }

    setLoading(true); setLiveSync(false); setData(null);
    const unsub = subscribeToRaidModuleState(teamId, cfg.key, snap => {
      setData(snap); setLoading(false); setLiveSync(true); setLastUpdate(new Date());
    });
    fetchRaidModuleState(teamId, cfg.key).then(d => { if (d) { setData(d); setLoading(false); } }).catch(() => {});
    return () => unsub();
  }, [teamId, cfg.key, cfg.title, BOSSES]);
  ...
  const currentBoss = useMemo(
    () => BOSSES.find(b => b.id === activeBoss) || BOSSES[0],
    [BOSSES, activeBoss],
  );
  ...
  return (
    <AppShell teamId={teamId}>
      ...
      {loading ? (
        ...
        <EmptyState icon={cfg.empty} title={`No ${cfg.title} assignments published yet`} message={`The raid leader hasn't published ${cfg.title} assignments yet. Check back soon.`} />
      ) : (
