          ...
          return (
            <div key={discordId} style={{
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 9, color: "#4ade80", letterSpacing: "0.1em", marginBottom: 5 }}>📅 TUESDAY</div>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      fontFamily: "'Cinzel', serif", fontSize: 11, outline: "none", boxSizing: "border-box",
                    }}
                  />
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 9, color: "#60a5fa", letterSpacing: "0.1em", marginBottom: 5 }}>📅 THURSDAY</div>
...
export default function AdminView({ teamId, teamName }) {
  ...
  const [sheetEditStatus, setSheetEditStatus] = useState("idle");
  const [combatLogEditUrl,    setCombatLogEditUrl]    = useState("");
  const [combatLogEditStatus, setCombatLogEditStatus] = useState("idle");
  ...
  const handleWclSubmit = useCallback(async () => {
    ...
    if (reportCode) {
      ...
      if (fetched) { autoDate = fetched; setRaidDate(fetched); }
    }
    ...
    const combatLogUrl = rawCombatLog ? rawCombatLog.replace(/\/(edit|view|htmlview|pub)(\?.*)?$/, "/htmlview") : null;
    ...
    try {
      ...
      if (viewingSnap) {
        ...
        if (Object.keys(updates).length) await updateSnapshot(teamId, viewingSnap, updates);
      ...
      setWclSubmitUrl(""); setSheetSubmitUrl(""); setCombatLogSubmitUrl("");
  ...
  }, [wclSubmitUrl, sheetSubmitUrl, combatLogSubmitUrl, viewingSnap, teamId, roster, rosterTue, rosterThu, assignments, textInputs, raidDate, raidLeader, specOverrides, dividers]);
  ...
  const handleImportJSON = (text, night) => {
    try {
      ...
      if (otherNightSlots.length > 0) {
        ...
        const tueDividers = night === "tue" ? divs          : [];
        const thuDividers = night === "thu" ? divs          : [];
        setPendingImportQueue({ tueSlots, thuSlots, tueDividers, thuDividers });
      } else {
        const queue = night === "tue"
          ? { tueSlots: slots, thuSlots: [], tueDividers: divs, thuDividers: [] }
          : { tueSlots: [], thuSlots: slots, tueDividers: [], thuDividers: divs };
  ...
  // ── Commit import — evicts stale/manual/legacy players, preserves assignments ──
  const commitImport = useCallback((queue, resolvedConflicts) => {
    ...
    setRoster(prev => {
      // Build map from previous roster, preserving wclName overrides
      const byId = new Map(prev.map(p => [p.id, p]));

      // ── FIX: evict all stale players — night-specific, manual, and legacy ──
      ...
      for (const [id, p] of byId) {
        if (
          nightsBeingImported.has(p.karaNight) || // old night-specific players
          p.manual ||                              // manually added characters
          !p.karaNight                             // legacy players with no night tag
        ) byId.delete(id);
      }

      // Merge new slots, preserving wclName from existing entries
      ...
      // ── FIX: drop assignments for evicted players, keep the rest ──
      setAssignments(prev => {
        const next = {};
  ...
  // Conflict detection runs when pendingImportQueue has both nights staged
  useEffect(() => {
  ...
  const handleCopyDiscord = () => {
    ...
    [["📅 TUESDAY", KARA_TUE_TEAMS], ["📅 THURSDAY", KARA_THU_TEAMS]].forEach(([nightLabel, teams]) => {
      const nightHasAny = teams.some(team => [...team.g1, ...team.g2].some(r => assignments[r.key]));
      ...
      {roster.length === 0 ? (
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
          ...
          <div style={{
            ...
            height: "100%", overflow: "hidden", zIndex: 10,
          }}>
            ...
            <div style={{ borderTop: "1px solid #1a1a2a", flexShrink: 0, display: "flex", flexDirection: "column", minHeight: 0, maxHeight: parsesOpen ? "50%" : "auto", overflow: "hidden" }}>
              ...
              {parsesOpen && (() => {
                ...
                const rows = players.map(p => {
                  ...
                  return { ...p, kara: wclScores[lookupName]?.kara ?? null, gruulMags: wclScores[lookupName]?.gruulMags ?? null };
                });
                ...
                return (
                  <div style={{ padding: "0 8px 8px", overflowY: "auto", flex: 1 }}>
                    ...
                    {rows.map(p => {
                      ...
                      return (
                        <div key={p.id} style={{ display: "flex", alignItems: "center", padding: "3px 4px", borderBottom: "1px solid #ffffff06" }}>
                          <span style={{ flex: 1, fontSize: 11, color: pColor, fontFamily: "'Cinzel', serif", overflow: "hidden", minWidth: 0 }}>
                            <AdminWclNameEditor player={p} onChange={wclName => handleWclNameChange(p.id, wclName)} />
                          </span>
                          <span style={{ width: 32, textAlign: "center", fontSize: 11, fontWeight: 700, fontFamily: "monospace", color: karaColor || "#444" }}>
                            {p.kara != null ? Math.round(p.kara) : "—"}
                          </span>
                          <span style={{ width: 36, textAlign: "center", fontSize: 11, fontWeight: 700, fontFamily: "monospace", color: gmColor || "#444" }}>
                              ...
                              {deleteSelected.size > 0 && (
                                <button
                                  onClick={async () => {
                                    ...
                                    try {
                                      for (const id of deleteSelected) await deleteSnapshot(teamId, id);
          ...
          <div style={{ flex: 1, padding: "14px 16px", minWidth: 0, overflowY: "auto", height: "100%" }}>
            ...
            {isLocked && viewSnap && (
              <div style={{ marginBottom: 12, padding: "10px 12px", background: "#0a0820", border: "1px solid #a78bfa44", borderRadius: 6 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 8 }}>
                  <span style={{ fontSize: 10, color: "#a78bfa", fontFamily: "'Cinzel', serif" }}>🔒 This week is locked</span>
                  {viewSnap.wclReportUrl && (
                    <a href={viewSnap.wclReportUrl} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: "#60a5fa", fontFamily: "'Cinzel', serif", textDecoration: "none" }}>📋 WarcraftLogs →</a>
                  )}
                  {viewSnap.sheetUrl && (
                    <a href={viewSnap.sheetUrl} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: "#4ade80", fontFamily: "'Cinzel', serif", textDecoration: "none" }}>📊 RPB Sheet →</a>
                  )}
                  {viewSnap.combatLogUrl && (
                    <a href={viewSnap.combatLogUrl} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: "#f59e0b", fontFamily: "'Cinzel', serif", textDecoration: "none" }}>⚔ Combat Log →</a>
                  )}
                </div>
                <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 8 }}>
                  <input value={sheetEditUrl} onChange={e => setSheetEditUrl(e.target.value)}
                  ...
                  <button
                    onClick={async () => {
                      ...
                      if (!raw) return;
                      if (!raw.includes("docs.google.com/spreadsheets")) { alert("Please paste a Google Sheets URL"); return; }
                      ...
                      try {
                        ...
                        setSheetEditUrl(""); setSheetEditStatus("saved");
                        setTimeout(() => setSheetEditStatus("idle"), 3000);
                      } catch (e) { console.error("Sheet URL save failed", e); setSheetEditStatus("idle"); }
                    }}
                    disabled={!sheetEditUrl.trim() || sheetEditStatus === "saving"}
                    style={btn(sheetEditStatus === "saved" ? "#0a200a" : "#0a1a10", sheetEditStatus === "saved" ? "#4ade8066" : "#4ade8033", "#4ade80")}
                  >
                    {sheetEditStatus === "saving" ? "Saving…" : sheetEditStatus === "saved" ? "✓ Saved!" : "💾 Save RPB"}
                  </button>
                </div>
                <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 6 }}>
                  <input value={combatLogEditUrl} onChange={e => setCombatLogEditUrl(e.target.value)}
                    placeholder={viewSnap.combatLogUrl ? "⚔ Update Combat Log URL…" : "⚔ Paste Combat Log Analytics URL…"}
                    style={{ flex: 1, background: "#080810", border: "1px solid #2a2a4a", borderRadius: 4, color: "#ccc", padding: "4px 10px", fontFamily: "'Cinzel', serif", fontSize: 10, outline: "none" }} />
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setCombatLogEditUrl(""); setCombatLogEditStatus("saved");
                        setTimeout(() => setCombatLogEditStatus("idle"), 3000);
                      } catch (e) { console.error("Combat log URL save failed", e); setCombatLogEditStatus("idle"); }
                    }}
                    disabled={!combatLogEditUrl.trim() || combatLogEditStatus === "saving"}
                    style={btn(combatLogEditStatus === "saved" ? "#1a0a00" : "#100800", combatLogEditStatus === "saved" ? "#f59e0b66" : "#f59e0b33", "#f59e0b")}
                  >
            ...
            {/* General assignments — hidden on Kara tab */}
            {activeTab !== "kara" && (
              <div style={{ marginBottom: 12, display: "flex", gap: 0, background: "#0a0a12", border: "1px solid #1e1e3a", borderRadius: 8, overflow: "hidden" }}>
                <div style={{ flex: 1, borderRight: "1px solid #1e1e3a" }}>
                  ...
                  {GENERAL_CURSES.map(row => (
                    <AssignmentRow key={row.key} rowCfg={row} assignedIds={viewAssignments[row.key]}
                      textValues={viewTextInputs} roster={viewRoster}
                      onDrop={isLocked ? null : handleDrop} onClear={isLocked ? null : handleClear}
                      onTextChange={isLocked ? null : handleTextChange} onDragStart={isLocked ? null : handleDragStart}
                      assignments={viewAssignments} compact />
                  ))}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ padding: "6px 12px", borderBottom: "1px solid #1e1e3a" }}>
            ...
            {activeTab === "kara" && (
              <AdminKaraSection
                rosterTue={rosterTue} rosterThu={rosterThu}
                viewAssignments={viewAssignments} isLocked={isLocked}
                handleDrop={handleDrop} handleClear={handleClear}
                handleDragStart={handleDragStart} handleSpecCycle={handleSpecCycle}
                discordCopiedTue={discordCopiedTue} discordCopiedThu={discordCopiedThu}
                setDiscordCopiedTue={setDiscordCopiedTue} setDiscordCopiedThu={setDiscordCopiedThu}
